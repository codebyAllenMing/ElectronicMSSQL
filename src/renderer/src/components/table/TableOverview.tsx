...
export default function TableOverview({ database, onSelectTable }: Props): JSX.Element {
  ...
  useEffect(() => { setSelected(new Set()) }, [schemaFilter])

  // Reset filter when database changes
  useEffect(() => { setSchemaFilter(null) }, [database])
  ...
  const handleExport = async (): Promise<void> => {
    ...
    try {
      ...
      const suggestedName =
        selectedTables.length === 1 ? `${selectedTables[0].tableName}.sql` : `${database}_schema.sql`
            ...
            {showSchemaDropdown && (
              <div className="absolute left-0 top-full mt-1 z-20 min-w-[180px] rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg py-1">
                ...
                <label className="flex items-center gap-2.5 px-3 py-1.5 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  />
                  <span className="text-xs text-gray-700 dark:text-gray-300">All schemas</span>
                </label>
                ...
                {schemas.map((schema) => (
                  ...
                  >
                    <Checkbox
                      checked={schemaFilter === null || schemaFilter.has(schema)}
                      indeterminate={false}
                      onChange={() => toggleSchema(schema)}
                    />
                    <span className="text-xs text-gray-700 dark:text-gray-300">{schema}</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
            <tr>
              ...
              {schemas.length > 1 && (
                <th className="px-4 py-2 text-left font-medium text-gray-500 dark:text-gray-400">Schema</th>
              )}
              ...
              >
                <td className="w-10 px-4 py-2" onClick={(e) => e.stopPropagation()}>
                  <Checkbox
                    ...
                    onChange={(checked) => toggleOne(table.tableSchema, table.tableName, checked)}
                  />
                </td>
                <td className="px-4 py-2 text-gray-800 dark:text-gray-200">{table.tableName}</td>
                {schemas.length > 1 && (
                  <td className="px-4 py-2 text-gray-400 dark:text-gray-500 text-xs">{table.tableSchema}</td>
                )}
