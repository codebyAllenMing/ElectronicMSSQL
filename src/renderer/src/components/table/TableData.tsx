...
export default function TableData({ database, tableSchema, tableName }: Props): JSX.Element {
  ...
  const handleCellDoubleClick = (val: unknown): void => {
    ...
    const text = val instanceof Date ? val.toISOString() : typeof val === 'object' ? JSON.stringify(val) : String(val)
  ...
  useEffect(() => { setPage(1) }, [limit, tableName])
  ...
  const onResizeStart = useCallback((col: string, e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    resizing.current = { col, startX: e.clientX, startWidth: colWidths[col] ?? DEFAULT_COL_WIDTH }
      ...
      <div className="flex items-center gap-4 px-4 py-2 border-b border-gray-100 dark:border-gray-800 shrink-0">
        ...
        {total !== null && (
          <span className="text-xs text-gray-400">
            Total: <span className="text-gray-600 dark:text-gray-300">{total.toLocaleString()}</span> rows
          </span>
        )}

        {totalPages !== null && (
          <div className="ml-auto flex items-center gap-1">
            <button onClick={() => setPage(1)} disabled={page === 1 || loading} className="px-2 py-1 text-xs rounded border border-gray-200 dark:border-gray-700 disabled:opacity-30 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:cursor-not-allowed">«</button>
            <button onClick={() => setPage((p) => p - 1)} disabled={page === 1 || loading} className="px-2 py-1 text-xs rounded border border-gray-200 dark:border-gray-700 disabled:opacity-30 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:cursor-not-allowed">‹</button>
            <span className="px-3 py-1 text-xs text-gray-600 dark:text-gray-300">{page} / {totalPages}</span>
            <button onClick={() => setPage((p) => p + 1)} disabled={page === totalPages || loading} className="px-2 py-1 text-xs rounded border border-gray-200 dark:border-gray-700 disabled:opacity-30 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:cursor-not-allowed">›</button>
            <button onClick={() => setPage(totalPages)} disabled={page === totalPages || loading} className="px-2 py-1 text-xs rounded border border-gray-200 dark:border-gray-700 disabled:opacity-30 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:cursor-not-allowed">»</button>
          </div>
        )}
      </div>
              ...
              <tr>
                {orderedCols.map((col) => (
                  <th
                    ...
                    onDragEnd={onDragEnd}
                    style={{ width: colWidths[col] ?? DEFAULT_COL_WIDTH, position: 'relative' }}
